...
| 4 Analytics | [04-analytics.md](04-analytics.md) | Postgres · ClickHouse | confirm → pivot kiến trúc |
| 5 Match history | [05-match-history.md](05-match-history.md) | Postgres · ScyllaDB | "nhanh hơn mọi mặt" → contract |

## Chuẩn bị chung (trước buổi)

- `npm run db:up` **sớm** — 5 container; **ScyllaDB ~20-30s**, **ClickHouse ~10s** khởi động chậm nhất.
- **Rehearsal 1 lần mỗi demo** để warm pool/cache → số live ổn định, không cold-start nhảy số. Demo 2 & 5 nhớ `:seed` trước.
...
| 4 Analytics | ClickHouse nhanh hơn cho aggregate | đúng (~10-46x) — nhưng **điểm là tách OLTP**, không phải tốc độ | CONFIRM (pivot) |
| 5 Match history | Scylla nhanh hơn mọi mặt | write thắng · read **ngang** · ad-hoc **từ chối** | BUST một phần |
