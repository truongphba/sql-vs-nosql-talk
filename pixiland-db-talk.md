...
- Resource occupancy — 1 claim / reward pool (event spike: mining pool, chest chung…)
...
|---|---|
| Resource occupancy (claim reward chung) | Race condition · coordination ngắn hạn |
...
|---|---|---|---|
| 1 Reward claim | `gamedb` · bảng `shared_rewards` | key `reward:1` | — |
| 2 Hero config | `gamedb_norm` + `gamedb_jsonb` (2 DB riêng) | — | DB `gamedb` · `users` / `wallets` / `heroes` |
| 3 Leaderboard | `gamedb` · `leaderboard` + `battle_results` | key `lb` | — |
| 4 Match history | `gamedb` · `matches` | — | — |
| 5 Analytics | `gamedb` · `battle_logs` | — | — |

> Demo 5 thêm **ClickHouse** (container, DB `default` · bảng `battle_logs`); Demo 4 thêm **ScyllaDB** (container, keyspace `gamedb` · bảng `matches_by_player`). Cả hai là engine riêng, không nằm trong cột Postgres/Redis/Mongo ở trên.

**Không xung đột:** demo 1 và 3 dùng chung DB `gamedb` nhưng bảng/key khác nhau; demo 2 tách 2 DB Postgres để so dung lượng embed vs normalized.

**Khi nào `npm run db:down -v`:** rehearsal sạch, lần đầu tạo `gamedb_norm` / `gamedb_jsonb` (init script chỉ chạy khi volume mới), hoặc sau khi đổi `docker/postgres-init/`.
...
Một reward pool (chest chung, mining pool, quà event…) chỉ cho **1 user claim** tại một thời điểm. Khi event bắt đầu, hàng trăm request cùng claim một pool còn trống trong vài millisecond.

**Scenario:** 100 user → 1 reward pool → cùng 1 lúc → chỉ 1 người được thắng.
...
# SQL chạy tay trên DataGrip (diễn race + FOR UPDATE 2 session + pg_locks): demos/01-reward-claim/queries.sql
```
...
Đọc `claimer_id`, thấy NULL thì update. Vấn đề: giữa lúc đọc và lúc write, nhiều request cùng pass check và cùng ghi đè nhau. Kết quả: nhiều user cùng "thắng" — oversubscribed, bug nghiêm trọng.
...
Redis SET NX không phải silver bullet. Nếu server crash sau khi Redis set key nhưng trước khi ghi vào PostgreSQL → state lệch: Redis nói reward đã bị claim, PostgreSQL không biết gì.
...
PostgreSQL UPDATE → ghi truth, audit log
    ↓
Khai thác / nhận reward bắt đầu ✓
    ↓ 30 phút sau
Redis TTL expire + PostgreSQL cleanup
```

> Vậy còn crash giữa hai bước thì sao? **TTL chính là cái cứu.** Redis lock tự hết hạn sau 30 phút nên không kẹt vĩnh viễn — xấu nhất là một reward pool bị "khoá nhầm" tạm thời rồi tự mở. Và vì **PostgreSQL là truth**, khi hai bên lệch thì đối soát theo PostgreSQL chứ không theo Redis. Độ lệch luôn bị chặn trên (bounded), không tích luỹ. (Cùng nguyên tắc "PostgreSQL thắng khi lệch" sẽ gặp lại ở Demo 3.)
...
**So sánh:** PostgreSQL thuần (`gamedb_norm`) · PostgreSQL JSONB (`gamedb_jsonb`) · MongoDB (`gamedb`)
...
Kỳ vọng định tính (100K hero): `gamedb_jsonb` **lớn hơn** `gamedb_norm` (~1.5–1.7×) vì duplicate snapshot trong `config`; `users`/`wallets` gần bằng nhau. Mongo embed tương tự nhưng BSON gọn hơn JSONB text.
...
### So sánh 2 approach — cùng INSERT battle mỗi trận

Cả hai đều ghi `battle_results` (audit). Khác nhau ở **chỗ cập nhật rank** và **chỗ đọc top-N**:

**01 — PG battle + UPSERT leaderboard** *(mọi thứ trên PostgreSQL)*

Mỗi trận: `INSERT battle_results` + `UPSERT leaderboard`. UI đọc `ORDER BY score DESC LIMIT N` từ PG.

- ✓ Một DB, đơn giản vận hành; có audit trail
- ✗ Mỗi trận = 2 write nóng trên PG (battle + rank index)
- ✗ Display đọc cùng bảng đang bị flood → read/write tranh nhau lúc spike

**02 — PG battle + Redis Sorted Set** *(tách read model — pattern production)*

Mỗi trận: `INSERT battle_results` (PG) + `ZINCRBY` (Redis). UI **chỉ** đọc top-N từ Redis; cuối season reward từ `battle_results`.

- ✓ Cùng audit trail; PG không phục vụ top-N display
- ✓ Write throughput nhanh hơn (~1.6x ở demo); spike display không tranh index leaderboard trên PG
- ✗ Hai hệ thống — cần rebuild Redis từ PG khi lệch

### Benchmark — 60K trận + top-10 query (idle & 40 reader spike)

| | PG battle + UPSERT LB | PG battle + Redis LB |
|---|---|---|
| 60K trận (throughput) | ~5.6s | ~3.4s |
| top-10 read — lúc rảnh | ~0.2ms | ~0.2ms |
| top-10 read — dưới write spike | ~3.4ms | ~0.8ms |
| Rank update | PostgreSQL | Redis Sorted Set |
| Display read | PostgreSQL | Redis Sorted Set |

> Số đo thật (localhost, `npm run demo:3`). Spike = **50 writer + 40 client đồng thời refresh top-10** trong lúc flood trận; pool PG **40 connection** (nhỏ hơn tổng load) → monolith xếp hàng rõ hơn. **Cùng INSERT battle** — idle read gần nhau; spike **PG ~3–4ms vs Redis ~0.8ms** vì cột trái đọc+ghi rank cùng PG. Throughput split nhanh hơn ~1.6x. Đây là lý do tách display sang Redis dù PG có index tốt lúc rảnh.
>
> Reward cuối season: tính từ `battle_results` (cả hai approach đều có). Redis chỉ phục vụ display realtime.
...
**Slide SCALE-OUT · DIAGRAM ONLY** (sau storage layout, trước takeaway): split PG vs Scylla cluster — PG: 1 primary → partition table → sharding thủ công; Scylla: N1/N2/N3 + token bar (fragment reveal `+ NODE`). Nói: *"Không scale 3 container trên laptop — slide thay production cluster."*

**Slide STORAGE LAYOUT** (sau benchmark + contract, trước scale-out): heap + index (PG) vs partition + clustering (Scylla). Cột giống DataGrip — khác cách data nằm. Nói sau terminal: *"Vừa thấy số — giờ giải tại sao ad-hoc Scylla từ chối."*
...
**Contention scale ≠ data volume scale.** Resource claim — bottleneck không phải vì nhiều data, mà vì nhiều request tranh cùng một row. Thêm read replica không giải quyết được write contention.
...
| Coordination ngắn hạn / atomic? | **Redis** | reward claim lock, cooldown, rate limit |
...
Redis             → reward claim lock, resource counter idle     (coordination)
...
| 4 | Demo 1 — Shared reward claim | 12–15 phút |
