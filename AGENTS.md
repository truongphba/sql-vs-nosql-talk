...
# CQL/SQL chạy tay (DataGrip): demos/04-analytics/queries.sql
npm run demo:5       # match history (wide-column): Postgres vs ScyllaDB (full)
npm run demo:5:seed  # ghi 200K event; rồi demo:5:read / demo:5:contract
...
- Demo 1–5 đã có code chạy thật qua `npm run db:up` rồi `npm run demo:1..5`. Demo 1, 2, 5 tách script nhỏ để demo live (`demo:1:naive|for-update|redis|all`; `demo:2:seed|layer1|layer2|lock`; `demo:5:seed|read|contract`); có `queries.sql`/`.cql` mỗi demo để chạy tay trên DataGrip. Demo 5 dùng ScyllaDB (wide-column) — container nặng, khởi động chậm.
