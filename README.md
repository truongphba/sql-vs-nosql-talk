# SQL vs NoSQL — DB Talk

Tài liệu + slide + **demo chạy thật** cho buổi tech talk *"SQL vs NoSQL — chọn storage theo workload, consistency và scale"*, dùng case study **một live game platform** (1M user / 50K DAU).
...
| Phần | Mô tả |
|---|---|
| [`pixiland-db-talk.md`](pixiland-db-talk.md) | Kịch bản đầy đủ (source of truth): Hook → Case study → bản đồ SQL/NoSQL → Demo 1–5 → Pitfalls → Decision Framework → Q&A |
...
| [`presentation/`](presentation/) | Deck reveal.js — 48 slide, no-build (mở `index.html` là chạy). Xem [presentation/README.md](presentation/README.md) |
...
- ScyllaDB (Demo 4) **nặng** (~2GB RAM, khởi động ~20-30s) — `db:up` chờ healthy lâu hơn; chạy `developer-mode` nên số chỉ minh hoạ
- ClickHouse (Demo 5): user/pass `game`/`game`, DB `default` — DataGrip nối `localhost:58123` (HTTP) hoặc `59000` (native) để xem data
...
npm run demo:1       # shared reward claim — in hướng dẫn từng case
...
npm run demo:3       # Leaderboard — PG monolith vs PG battle + Redis (cùng audit)
npm run demo:4       # Match history (wide-column) — Postgres vs ScyllaDB (full)
npm run demo:4:seed        # ghi 200K event (đo write throughput)
...
Mỗi demo tự reset state đầu run nên chạy lại nhiều lần đều nhất quán. **Một lần `db:up` cho cả buổi** — không cần restart Docker giữa các demo. Demo 2 dùng 2 DB Postgres (`gamedb_norm`, `gamedb_jsonb`); demo 1/3 dùng `gamedb`; demo 4 (match history) thêm keyspace `gamedb` trên ScyllaDB; demo 5 (analytics) thêm ClickHouse (DB `default`).
...
|---|---|---|---|
| **1** Shared reward claim | coordination / race | naive ~80–96 winner (FAIL) **& chậm nhất** · FOR UPDATE & Redis đúng 1 | naive vẫn lock ở UPDATE → FOR UPDATE **không** chậm nhất |
...
| **4** Match history | wide-column write-heavy | Scylla write ~2.3x · partition read **ngang** PG · ad-hoc Scylla **từ chối** | wide-column = hợp đồng khác, không "nhanh hơn mọi mặt" |
| **5** Analytics | OLAP scan | ClickHouse ~10–46x cho aggregate | Đúng như nghĩ — tách OLAP khỏi OLTP |

> Bài học: dùng đúng DB cho đúng workload, **và verify bằng benchmark** — đừng tin số trên blog người khác.

## Stack đề xuất (5 vai trò · 3 engine)

| Engine | Vai trò |
|---|---|
| PostgreSQL | transactional truth (user, ví in-game, vật phẩm, kết quả trận) |
| PostgreSQL JSONB | flexible config (hero, building) |
| Redis | coordination (reward claim lock) + read model (leaderboard Sorted Set) |
...
ScyllaDB/Cassandra (wide-column) cũng vậy — chỉ thêm khi match-history/event write-heavy vượt sức 1 node Postgres (xem Demo 4).
...
demos/01-05/             # demo code (demo 2 & 4 tách script nhỏ; có queries.sql/.cql)
