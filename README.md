...
- ClickHouse (Demo 5): user/pass `pixi`/`pixi`, DB `default` — DataGrip nối `localhost:58123` (HTTP) hoặc `59000` (native) để xem data
...
Mỗi demo tự reset state đầu run nên chạy lại nhiều lần đều nhất quán. **Một lần `db:up` cho cả buổi** — không cần restart Docker giữa các demo. Demo 2 dùng 2 DB Postgres (`pixiland_norm`, `pixiland_jsonb`); demo 1/3 dùng `pixiland`; demo 4 (match history) thêm keyspace `pixiland` trên ScyllaDB; demo 5 (analytics) thêm ClickHouse (DB `default`).
