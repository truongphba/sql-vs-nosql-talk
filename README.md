...
Mỗi demo tự reset state đầu run nên chạy lại nhiều lần đều nhất quán. **Một lần `db:up` cho cả buổi** — không cần restart Docker giữa các demo. Demo 2 dùng 2 DB Postgres (`pixiland_norm`, `pixiland_jsonb`); demo 1/3/4 dùng `pixiland`.
...
| **2** Hero config | xuôi vs ngược vs JOIN vs migration | Mongo thắng xuôi · SQL thắng JOIN/ngược · DX = không DDL | **JSONB sweet spot** |
